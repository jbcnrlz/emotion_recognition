...
from loss.EuclideanLoss import EuclideanLoss
from DatasetClasses.AffectNet import AffectNet
from helper.function import saveStatePytorch, printProgressBar, loadNeighFiles
from networks.ResnetEmotionHead import ResnetEmotionHeadClassifierAttention, SmallClassified
...
def train():
    ...
    modelAFD = SmallClassified(3).to(device)
    optimizerAFD = optim.Adam(modelAFD.parameters(), lr=args.learningRate)
    criterionAFD = nn.CrossEntropyLoss().to(device)

    modelSH = SmallClassified(2).to(device)
    optimizerSH = optim.Adam(modelSH.parameters(), lr=args.learningRate)
    criterionSH = nn.CrossEntropyLoss().to(device)
    ...
    for ep in range(args.epochs):
        ...
        for currBatch, currTargetBatch, pathfile in train_loader:
            ...
            featuresSH = torch.concat((features[predicted == 1],features[predicted == 3]),dim=0)
            gtSH = torch.tensor([0] * features[predicted == 1].shape[0] + [1] * features[predicted == 3].shape[0]).to(device)
            clSH = modelSH(featuresSH)
            shCELOSS = criterionSH(clSH,gtSH)
            featuresAFD = torch.concat((features[predicted == 6],features[predicted == 5],features[predicted == 4]),dim=0)
            gtAFD = torch.tensor([0] * features[predicted == 6].shape[0] + [1] * features[predicted == 5].shape[0] + [2] * features[predicted == 4].shape[0]).to(device)
            clAFD = modelAFD(featuresAFD)
            afdCELOSS = criterionAFD(clAFD,gtAFD)
            
            optimizer.zero_grad()
            loss.backward(retain_graph=True)
            optimizer.step()

            optimizerAFD.zero_grad()
            afdCELOSS.backward(retain_graph=True)
            optimizerAFD.step()

            optimizerSH.zero_grad()
            shCELOSS.backward(retain_graph=True)
            optimizerSH.step()

            lossAcc.append(loss.item())
            lossAvgSH.append(shCELOSS.item())
            lossAvgAFD.append(afdCELOSS.item())
            iteration += 1

        lossAvg = sum(lossAcc) / len(lossAcc)
        lossAvgSH = sum(lossAvgSH) / len(lossAvgSH)
        lossAvgAFD = sum(lossAvgAFD) / len(lossAvgAFD)
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                gtSH = torch.tensor([0] * features[predicted == 1].shape[0] + [1] * features[predicted == 3].shape[0]).to(device)
                clSH = modelSH(featuresSH)
                _, predSH = torch.max(clSH.data, 1)
                shCELOSS = criterionSH(clSH,gtSH)
                featuresAFD = torch.concat((features[predicted == 6],features[predicted == 5],features[predicted == 4]),dim=0)
                gtAFD = torch.tensor([0] * features[predicted == 6].shape[0] + [1] * features[predicted == 5].shape[0] + [2] * features[predicted == 4].shape[0]).to(device)
                clAFD = modelAFD(featuresAFD)
                _, predAFD = torch.max(clAFD.data, 1)
                afdCELOSS = criterionAFD(clAFD,gtAFD)

                lossValAFD.append(afdCELOSS)
                lossValSH.append(shCELOSS)
                ...
                for idx, p in enumerate(predicted):
                    if p in [0,2]:
                        correct += int(p == labels[idx])
                    elif p in [1,3]:
                        correct += int(predSH[currSH] == gtSH[currSH])
                        currSH += 1
                    else:
                        correct += int(predAFD[currAFD] == gtAFD[currAFD])
                        currAFD += 1
